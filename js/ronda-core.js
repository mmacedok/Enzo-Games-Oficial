...
(function (root, factory) {
    ...
    function colocarDesafio(jogo, predio, v, dif, pouso) {
        ...
        const y = tipo === 'baixo' || tipo === 'parede'
            ? predio.topo - t.h
            : predio.topo - 26 - t.h / 2; // pássaro e drone na altura da arma
        jogo.desafios.push({ tipo, x, y, w: t.w, h: t.h, base: y, vida: t.vida, fase: jogo.aleatorio() * Math.PI * 2 });
    }
    ...
    /** Um tiro, respeitando a cadência. Retorna true se atirou. */
    function atirar(jogo) {
        if (jogo.fase !== 'correndo' || jogo.recarga > 0) return false;
    ...
    function passo(jogo) {
        ...
        if (jogo.desafios.some((d) => colide(corpo, d))) return terminar(jogo, 'desafio');
    ...
    return {
        CONFIG, TIPOS, criarJogo, reiniciar, pular, atirar, avancar, pontos,
        alcanceDoPulo, alturaPulo, velocidadeEm, colide, hitboxJogador,
    };
});
