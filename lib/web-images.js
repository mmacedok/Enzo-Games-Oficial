...
async function buildImages() {
    ...
    function walk(dir) {
        for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (item.name === 'web' || item.name.startsWith('.') || rel === 'assets/flappy/game' || rel === 'assets/flappy/originais') continue;
    ...
    for (const name of ['index.html', 'personagens.html', 'degustador.html']) {
        ...
        html = html.replace(/<img\b[^>]*>/g, tag => {
            ...
            return tag.replace('<img', `<img data-original="${original}" src="${info.variants[0].src}" srcset="${srcset}" sizes="(max-width: 600px) 90vw, 420px" width="${info.width}" height="${info.height}"`);
        });
